...
public class CalculatorImplArray implements Calculator{
    TwoWayPrice [][] prices = new TwoWayPrice[Market.values().length][Instrument.values().length];
    ...
    public TwoWayPrice applyMarketUpdate(MarketUpdate marketUpdate) {
        ...
        prices[updateMarket.ordinal()][updateInstrument.ordinal()] = updatePrice;
    ...
    TwoWayPrice calculateVWAP(Instrument instrument) {
        ...
        for (Market market: Market.values()) {
           var instrumentPrice = prices[market.ordinal()][instrument.ordinal()];
